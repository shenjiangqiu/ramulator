...
map<string, enum STTMRAM::Org> STTMRAM::org_map = {
    {"STTMRAM_512Mb_x4", STTMRAM::Org::STTMRAM_512Mb_x4},
    {"STTMRAM_512Mb_x8", STTMRAM::Org::STTMRAM_512Mb_x8},
    {"STTMRAM_512Mb_x16", STTMRAM::Org::STTMRAM_512Mb_x16},
    {"STTMRAM_1Gb_x4", STTMRAM::Org::STTMRAM_1Gb_x4},
    {"STTMRAM_1Gb_x8", STTMRAM::Org::STTMRAM_1Gb_x8},
    {"STTMRAM_1Gb_x16", STTMRAM::Org::STTMRAM_1Gb_x16},
    {"STTMRAM_2Gb_x4", STTMRAM::Org::STTMRAM_2Gb_x4},
    {"STTMRAM_2Gb_x8", STTMRAM::Org::STTMRAM_2Gb_x8},
    {"STTMRAM_2Gb_x16", STTMRAM::Org::STTMRAM_2Gb_x16},
    {"STTMRAM_4Gb_x4", STTMRAM::Org::STTMRAM_4Gb_x4},
    {"STTMRAM_4Gb_x8", STTMRAM::Org::STTMRAM_4Gb_x8},
    {"STTMRAM_4Gb_x16", STTMRAM::Org::STTMRAM_4Gb_x16},
    {"STTMRAM_8Gb_x4", STTMRAM::Org::STTMRAM_8Gb_x4},
    {"STTMRAM_8Gb_x8", STTMRAM::Org::STTMRAM_8Gb_x8},
    {"STTMRAM_8Gb_x16", STTMRAM::Org::STTMRAM_8Gb_x16},
};

map<string, enum STTMRAM::Speed> STTMRAM::speed_map = {
    {"STT_1600_1_2", STTMRAM::Speed::STT_1600_1_2},
    {"STT_1600_1_5", STTMRAM::Speed::STT_1600_1_5},
    {"STT_1600_2_0", STTMRAM::Speed::STT_1600_2_0},
};
...
void STTMRAM::init_speed() {
  ...
  case 1600:
    speed_entry.nRRD = ceil(speed_entry.nRRD * ((page == 1) ? 5 : 6) / 10.0);
    speed_entry.nFAW = ceil(speed_entry.nFAW * ((page == 1) ? 24 : 32) / 10.0);
    break;
  default:
    throw false;
  ...
  switch (speed_entry.rate) {
  case 1600:
    speed_entry.nXS = ceil(speed_entry.nXS *
                           ((chip == 512)       ? 80
                            : (chip == 1 << 10) ? 96
                            : (chip == 1 << 11) ? 136
                            : (chip == 1 << 12) ? 216
...
void STTMRAM::init_prereq() {
  // RD
  prereq[int(Level::Rank)][int(Command::RD)] = [](DRAM<STTMRAM> *node,
                                                  Command cmd, int id) {
  ...
  prereq[int(Level::Bank)][int(Command::RD)] = [](DRAM<STTMRAM> *node,
                                                  Command cmd, int id) {
    switch (int(node->state)) {
  ...
  // REF
  prereq[int(Level::Rank)][int(Command::REF)] = [](DRAM<STTMRAM> *node,
                                                   Command cmd, int id) {
  ...
  // PD
  prereq[int(Level::Rank)][int(Command::PDE)] = [](DRAM<STTMRAM> *node,
                                                   Command cmd, int id) {
    switch (int(node->state)) {
    case int(State::PowerUp):
      return Command::PDE;
  ...
  // SR
  prereq[int(Level::Rank)][int(Command::SRE)] = [](DRAM<STTMRAM> *node,
                                                   Command cmd, int id) {
    switch (int(node->state)) {
    case int(State::PowerUp):
      return Command::SRE;
...
void STTMRAM::init_rowhit() {
  // RD
  rowhit[int(Level::Bank)][int(Command::RD)] = [](DRAM<STTMRAM> *node,
                                                  Command cmd, int id) {
    switch (int(node->state)) {
    case int(State::Closed):
      return false;
...
void STTMRAM::init_rowopen() {
  // RD
  rowopen[int(Level::Bank)][int(Command::RD)] = [](DRAM<STTMRAM> *node,
                                                   Command cmd, int id) {
    switch (int(node->state)) {
    case int(State::Closed):
      return false;
...
void STTMRAM::init_lambda() {
  lambda[int(Level::Bank)][int(Command::ACT)] = [](DRAM<STTMRAM> *node,
                                                   int id) {
    node->state = State::Opened;
    node->row_state[id] = State::Opened;
  };
  lambda[int(Level::Bank)][int(Command::PRE)] = [](DRAM<STTMRAM> *node,
                                                   int id) {
    node->state = State::Closed;
    node->row_state.clear();
  };
  lambda[int(Level::Rank)][int(Command::PREA)] = [](DRAM<STTMRAM> *node,
                                                    int id) {
    for (auto bank : node->children) {
  ...
  lambda[int(Level::Rank)][int(Command::REF)] = [](DRAM<STTMRAM> *node,
                                                   int id) {};
  ...
  lambda[int(Level::Bank)][int(Command::RDA)] = [](DRAM<STTMRAM> *node,
                                                   int id) {
    node->state = State::Closed;
    node->row_state.clear();
  };
  lambda[int(Level::Bank)][int(Command::WRA)] = [](DRAM<STTMRAM> *node,
                                                   int id) {
    node->state = State::Closed;
    node->row_state.clear();
  };
  lambda[int(Level::Rank)][int(Command::PDE)] = [](DRAM<STTMRAM> *node,
                                                   int id) {
    for (auto bank : node->children) {
      if (bank->state == State::Closed)
        continue;
  ...
  lambda[int(Level::Rank)][int(Command::PDX)] =
      [](DRAM<STTMRAM> *node, int id) { node->state = State::PowerUp; };
  lambda[int(Level::Rank)][int(Command::SRE)] =
      [](DRAM<STTMRAM> *node, int id) { node->state = State::SelfRefresh; };
  lambda[int(Level::Rank)][int(Command::SRX)] =
      [](DRAM<STTMRAM> *node, int id) { node->state = State::PowerUp; };
}
